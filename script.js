...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bindCardFunctionality = (card, meal) => {
    ...
    rateStar.addEventListener('click', () => {
      ...
      const newRating = currentRating === 0 ? userRating : (currentRating + userRating) / 2;
  ...
  const fetchResultantRecipes = async (query) => {
    ...
    try {
      ...
      if (!data.meals) {
        resultCards.innerHTML = "<h2 class='error'>No recipes found. Try another search!</h2>";
        return;
      }
...
const openMenuPopup = async () => {
  try {
    ...
    // Generate HTML
    const recipeList = Object.entries(groupedRecipes).map(([category, meals]) => `
...
function validateForm() {
  ...
  if (recipeName.value.trim() === "") {
    ...
    recipeNameError.textContent = "Recipe name must be less than 50 characters.";
    recipeNameError.style.color = 'red';
    isValid = false;
  }
  ...
  if (ingredients.value.trim() === "") {
    ...
    ingredientsError.textContent = "Ingredients must be less than 200 characters.";
    ingredientsError.style.color = 'red';
    isValid = false;
  }
  ...
  if (instructions.value.trim() === "") {
    ...
    instructionsError.textContent = "Instructions must be less than 500 characters.";
    isValid = false;
  }
  return isValid;
}
...
closeShareRecipeBtn.addEventListener("click", () => shareRecipeContainer.style.display = "none");
...
const mealPlans = {
  vegan: {
    breakfast: ["Tofu Scramble", "Vegan Smoothie Bowl", "Chia Pudding with Berries", "Oatmeal with Almond Butter"],
    lunch: ["Lentil Salad", "Vegan Tacos", "Quinoa & Chickpea Salad", "Veggie Stir-Fry with Tofu"],
    dinner: ["Vegan Buddha Bowl", "Stuffed Sweet Potatoes", "Vegan Lasagna", "Vegetable Curry with Rice"]
  },
  vegetarian: {
    breakfast: ["Vegetarian Breakfast Burrito", "Greek Yogurt with Honey and Fruit", "Avocado Toast", "Vegetable Omelette"],
    lunch: ["Caprese Salad", "Vegetarian Wrap with Hummus", "Stuffed Bell Peppers", "Grilled Cheese Sandwich with Tomato Soup"],
    dinner: ["Vegetable Stir Fry", "Mushroom Risotto", "Vegetarian Chili", "Vegetarian Pizza"]
  },
  keto: {
    breakfast: ["Scrambled Eggs with Avocado", "Keto Pancakes", "Chia Pudding with Almond Milk", "Bacon and Eggs"],
    lunch: ["Chicken Salad with Avocado", "Zucchini Noodles with Pesto", "Grilled Salmon with Asparagus", "Keto Chicken Wrap"],
    dinner: ["Cauliflower Rice Stir-Fry", "Keto Chicken Alfredo", "Zucchini Lasagna", "Steak with Roasted Vegetables"]
  },
  "low-carb": {
    breakfast: ["Scrambled Eggs with Spinach", "Low-Carb Smoothie", "Egg Muffins", "Greek Yogurt with Nuts"],
    lunch: ["Grilled Chicken Salad", "Cauliflower Rice Bowl", "Lettuce Wraps with Turkey and Cheese", "Eggplant Parmesan"],
    dinner: ["Grilled Salmon with Broccoli", "Cauliflower Mash with Steak", "Low-Carb Pizza", "Chicken with Zucchini Noodles"]
  }
};
...
function generateMealPlan(diet) {
  ...
  localStorage.setItem('mealPlan', JSON.stringify({ breakfast, lunch, dinner }));
